...
class ReturnRefundItemHandler {
  ...
  initializeHandlers() {
    ...
    this.getItemsByReturn = this.importHandler("./get/by_return.ipc");
  ...
  importHandler(path) {
    try {
      ...
      console.warn(`[ReturnRefundItemHandler] Failed to load handler: ${path}`, error.message);
      return async () => ({
        status: false,
  ...
  async handleRequest(event, payload) {
    try {
      ...
      if (logger) {
        logger.info(`ReturnRefundItemHandler: ${method}`, { params });
      }
      ...
      switch (method) {
        ...
        case "getItemsByReturn":
          return await this.getItemsByReturn(params);
        case "getItemsByMeat":
      ...
      console.error("ReturnRefundItemHandler error:", error);
      if (logger) {
        logger.error("ReturnRefundItemHandler error:", error);
      }
      return {
        status: false,
...
const returnRefundItemHandler = new ReturnRefundItemHandler();

ipcMain.handle(
  "returnRefundItem",
  withErrorHandling(
    returnRefundItemHandler.handleRequest.bind(returnRefundItemHandler),
    "IPC:returnRefundItem"
  )
);

module.exports = { ReturnRefundItemHandler, returnRefundItemHandler };